...
print(f"Explained variance ratio of first 5 PCs: {explained_variance[:5]}") # adapted after obtaining optimal num of components

# for PCA plot
pca_df = pd.DataFrame(drug_scores[:, :5], columns=['PC1', 'PC2', 'PC3', 'PC4', 'PC5'], index= Y_matrix.index)

# Adding drug categories
...
target_loadings= pd.DataFrame(pca_targets.components_.T[:,:5], index= X_final.columns, columns= ['PC1', 'PC2', 'PC3', 'PC4', 'PC5'])
...
top_targets = target_loadings.sort_values(
    "magnitude", ascending=False).head(5)
...
# 2. Plot the main anticancer points
plt.scatter(
    X_scores_anticancer[:, 0],
    X_scores_anticancer[:, 1],
    color='#E23C4A', 
    alpha=1, 
    label='Noncancer drugs with anti-cancer potential',
    edgecolor='k', 
    s=30
)

# 3. Initialize an empty list to gather the text labels
texts = []

# 4. Loop to generate the text objects (No plt.annotate inside here!)
for i, txt in enumerate(anticancer_drugs):
    # Tip: You can change this to just `txt` if you want the full name now!
    label_text = f"{anticancer_drugs[i][:5]}" 
...
# 5. Call adjust_text ONCE out of the loop to clean them all up simultaneously
adjust_text(
    texts,
    arrowprops=dict(arrowstyle="->", color="gray", lw=0.5),
    # Force text to move away from points AND other text more aggressively
    force_text=(0.5, 1.0),  
    # Expands the bounding boxes conceptually so the algorithm spaces them wider
    expand=(1.5, 1.8),  
)
